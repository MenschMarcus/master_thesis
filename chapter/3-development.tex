...
Before the \emph{EditMode} for manipulating Areas, Hivents and Historical Changes in the HistoGlobe interface can be introduced in section \ref{sub:edit_mode}, the underlying operations have to be defined. That means: What kind of changes can be introduced by a user that wants to edit the content in the system? Throughout the development process, many interviews with historians and other researchers in humanities at University of Virginia were conducted, to understand their mental model about historical changes of countries.

Section \ref{sub:historical_geographic_operations} presented the five basic Historical Geographic Operations that can model all possible changes of countries in time and space. In the interviews it turned out that the HG Operations are not suitable to be used for human edit purposes, because of their low-level nature: The operations do not provide a straightforward way to create a new country on previously unclaimed land. Also changing the formal name of an Area is modelled by a \texttt{UNI} operation, which is not very intuitive. The interviews revealed the following six high-level operations for editing the countries on the map. They will be directly associated to an Hivent, so they are called \emph{Hivent Operations}.
...
\begin{description}
  ...
  \item[DIS -- Dissolve]
  one Area into several new Areas, manually setting their new territory and name.
  \item[CHB -- Change Borders]
  between two neighboring Areas by defining the territory that changes sides.
  \item[REN -- Rename]
  an Area and set a new formal and / or short name.
  \item[CES -- Cease]
  an Area by deleting it from the map, leaving unclaimed land behind.
\end{description}

Each of the operations can be expressed with a combination of the HG Operations. Therefore the Hivent Operations serve as an abstraction layer between the Hivent and the HG Operations. The conversion between the operations is presented in table \ref{tab:hivent_to_hg_operations}.
...
\begin{tabular}{l l l l l l}
  ...
  \bottomrule
\end{tabular}
\caption{Expression of Hivent Operations with Historical Geographic Operations}
...
what is to be said about that great thing?

...
A time-based approach addresses exactly those shortcomings: They explicitly represent events or processes in the data model and associate all entities that change according to them. One example of this approach is the \emph{Event-Based Spatio-Temporal Data Model} (ESTDM) by
\cite{peuquet95}:
At one defined time point $t_b$, the \emph{base state}, a snapshot gets stored. From that moment on only changes relative to that base state are model. This happens in the form of events, the organizing entity of the data model. An event has a time stamp $t_i$ and a list of spatial and attribute changes associated with it. In the model for raster data introduced by \cite{peuquet95}, such a discrete change has raster components associated to it that state which raster cells change their value in this change. The events are chronologically ordered and stored in a doubly-linked list: each event knows its preceding and succeeding event.
% TODO: how does it EXACTLY work?

This model allows for efficient change tracking, and handling, due to simple insertion and deletion into and from the event list.

Just like the Simple Time-Stamping method, this approach is only suitable for discrete changes but not for grdual changes. The main constraint for this thesis is that \cite{peuquet95} have developed this model only for raster data. ``The transformation of the ESTDM to a vector-based system requires a substantial redesign of event components''
\cite[p. 11]{pelekis04stdms}.
...
% TODO: Find a model that conceptualized that for vector data
...
extension of ebstdm
To trace historical relationships, a change can also store type of change:
Creation, Alteration, Cessation, Reincarnation, Split/Deduction, Merge/Annexation

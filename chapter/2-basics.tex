...
To understand, what an Historical Geographic Information System is, it is important to understand the four parts of the word: The research fields \emph{history} and \emph{geography} and the concepts of \emph{information} an \emph{systems}.
...
A model tries to replicate a part of the real world. A data model abstracts a part of the world, identifies the most essential elements and their relation to each other and conceptualizes them e.g. in an \emph{Entity Relationship Model}. In an HGIS, the data model should contain entities and relations to explain spatial-temporal phenomena in the real world. There are five dimensions involved:

\begin{table}[ht]
\begin{center}
\begin{tabular}{l l l l}
    \toprule
    question & dimension & variable & description \\
    \midrule
    where? & location & lat & latitude \\
    where? & location & lng & longitude \\
    where? & location & h & height \\
    when? & time & t & time \\
    what? & attribute & a & set of attributes \\
    \bottomrule
\end{tabular}
\caption{Five dimensions of an Historical Geographic Information System}
\label{tab:dimensions_hgis}
\end{center}
\end{table}

%TODO: visualize triadic framework instead?

Based on the theory of the \emph{Triadic Framework}, these dimensions can change independently from each other, so the information system has to account for that
\cite[p. 53]{ott2001time}.

The remaining part of the section will introduce data models for space and time separetly and finally give an overview about joint spatio-temporal data models.

% ------------------------------------------------------------------------------
\subsection{Spatial Data Models} % (fold)
\label{sub:spatial_data_models}
...
measured by scientists in the field of \emph{geodesy} is rather complicated. In the Babylonian Empire ($\approx$ 2000-539 BC) the theory of the Earth being a flat disc surrounded by an infinite body of water
...
% ------------------------------------------------------------------------------
\subsubsection{Geographic Coordinate System} % (fold)
...
% subsubsection geographic_coordinate_system (end)

% ------------------------------------------------------------------------------
\subsubsection{Geodetic datum} % (fold)
\label{ssub:geodetic_datum}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{graphics/basics/raster_vector}
...
% section raster_vs_vector_model (end)

% subsection spatial_data_models (end)

% ------------------------------------------------------------------------------
\subsection{Temporal Data Models} % (fold)
\label{sub:temporal_data_models}
...
% subsection temporal_data_models (end)
...
The goal of a HGIS is to show the development of geo-objects over time. Developments are driven by \emph{changes} to the state of an object. According to the Triadic Framework time, space and attribute can change independently from each other. However, in order to trace spatial and attributal changes over time, there has to be a relation between the dimensions.
...
To represent spatio-temporal data in an information system, there are three different approaches: location-based, entity-based and time-based representations.

% ------------------------------------------------------------------------------
\subsubsection{Location-Based Data Models} % (fold)
\label{ssub:location_based_data_models}
...
% subsubsection location_based_data_models (end)

% ------------------------------------------------------------------------------
\subsubsection{Entity-Based Data Models} % (fold)
\label{ssub:entity_based_data_models}



% subsubsection entity_based_data_models (end)

% ------------------------------------------------------------------------------
\subsubsection{Time-Based Data Models} % (fold)
\label{ssub:time_based_data_models}

Since time is the main organizing dimension, time-based spatio-temporal changes of entities can as well be separated into two groups
\cite{peuquet99}:

\begin{description}
  \item[Discrete] Based on the idea of a \emph{state machine}, an entity stays in its state until an event occurs that suddenly changes the state at a discrete time point. As an example, the age of human beings changes once per year or their birthday.
  \item[Continuous] An entity gradually changes according to a continuous function, either in a uniform or a irregular manner. Its development is driven by a process, e.g. the melting of the polar ice caps.
\end{description}
...
% subsubsection time_based_data_models (end)
...
History-graph model
=> used for mixed types of objects
...
% ------------------------------------------------------------------------------
\subsection{Timelines} % (fold)
\label{sub:timelines}

% subsection timelines (end)
